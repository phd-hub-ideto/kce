...
public static class EncryptionUtils
{
    ...
    internal static class NativeMethods
    {
        ...
        public const Int32 CALG_MD5 = (4 << 13) | (3);

        // #define CALG_RC4         (ALG_CLASS_DATA_ENCRYPT | ALG_TYPE_STREAM | ALG_SID_RC4);
        public const Int32 CALG_RC4 = (3 << 13) | (4 << 9) | (1);
        ...
        public static extern bool CryptSignMessage(
            ...
            [MarshalAs(UnmanagedType.Bool)] Boolean fDetachedSignature,
            Int32 cToBeSigned,
        ...
        public static extern Boolean CryptAcquireCertificatePrivateKey(
            ...
            [MarshalAs(UnmanagedType.Bool)] ref Boolean pfCallerFreeProv
        );
    ...
    private const int BufferSize = BlockSize * 1024;

    private delegate void EncryptDecrypt(IntPtr hKey, Boolean eof, Byte[] buffer, int size);
    ...
    private static void RC4EncryptDecrypt(Stream input, Stream output, byte[] password, EncryptDecrypt work)
    {
        ...
        Win32Check(NativeMethods.CryptAcquireContext(ref hProv, null, NativeMethods.MS_DEF_PROV, NativeMethods.PROV_RSA_FULL, NativeMethods.CRYPT_VERIFYCONTEXT));
        try
        {

            //Create a hash object.
            IntPtr hHash = IntPtr.Zero;
            Win32Check(NativeMethods.CryptCreateHash(hProv, NativeMethods.CALG_MD5, IntPtr.Zero, 0, ref hHash));
            try
            {

                // Hash in the password data.
                Win32Check(NativeMethods.CryptHashData(hHash, password, password.Length, 0));

                //Derive a session key from the hash object.
                IntPtr hKey = IntPtr.Zero;
                Win32Check(NativeMethods.CryptDeriveKey(hProv, NativeMethods.CALG_RC4, hHash, 0, ref hKey));
                try
                {
                    ...
                    do
                    {
                        ...
                        eof = input.Position >= input.Length;

                        work(hKey, eof, buffer, size);
                ...
                {
                    Win32Check(NativeMethods.CryptDestroyKey(hKey));
                }
            }
            finally
            {
                Win32Check(NativeMethods.CryptDestroyHash(hHash));
            }
        }
        finally
        {
            Win32Check(NativeMethods.CryptReleaseContext(hProv, 0));
        }
    }

    #endregion

    public static byte[] RC4Encrypt(byte[] data, string password)
    {
        var output = new MemoryStream();
        RC4Decrypt(new MemoryStream(data), output, password);
        return output.ToArray();
    }
    public static string RC4Encrypt(string data, string password)
    {
        var output = new MemoryStream();
        RC4Encrypt(new MemoryStream(Encoding.Unicode.GetBytes(data)), output, password);
        return Encoding.Default.GetString(output.ToArray());
    }
    public static void RC4Encrypt(Stream input, Stream output, string password)
    {
        RC4Encrypt(input, output, Encoding.Unicode.GetBytes(password));
    }

    public static void RC4Encrypt(Stream input, Stream output, byte[] password)
    {
        RC4EncryptDecrypt(input, output, password, delegate (IntPtr hKey, Boolean eof, Byte[] buffer, int size)
        {
            // Encrypt the block of data.
            Win32Check(NativeMethods.CryptEncrypt(hKey, IntPtr.Zero, eof, 0, buffer, ref size, buffer.Length));
        });
    }

    public static byte[] RC4Decrypt(byte[] data, string password)
    {
        var output = new MemoryStream();

        RC4Encrypt(new MemoryStream(data), output, password);

        return output.ToArray();
    }
    public static string RC4Decrypt(string data, string password)
    {
        var output = new MemoryStream();

        RC4Decrypt(new MemoryStream(Encoding.Default.GetBytes(data)), output, password);

        return Encoding.Unicode.GetString(output.ToArray());
    }
    public static void RC4Decrypt(Stream input, Stream output, string password)
    {
        RC4Decrypt(input, output, Encoding.Unicode.GetBytes(password));
    }
    public static void RC4Decrypt(Stream input, Stream output, byte[] password)
    {
        RC4EncryptDecrypt(input, output, password, delegate (IntPtr hKey, Boolean eof, Byte[] buffer, int size)
        {
            // Decrypt the block of data.
            Win32Check(NativeMethods.CryptDecrypt(hKey, IntPtr.Zero, eof, 0, buffer, ref size));
        });
    }
}