...
public static class DateTimeExtensions
{
    ...
    /// <summary>
    /// Sets the hour on the specified DateTime value, and the minutes and seconds to zero.
    /// </summary>
    ...
    {
        return date.SetTime(hours, 0, 0);
    }

    /// <summary>
    /// Sets the hour and minutes on the specified DateTime value, and the seconds to zero.
    /// </summary>
    /// <param name = "date">The base date.</param>
    /// <param name = "hours">The hours to be set.</param>
    ...
    {
        var ts = datetime.Subtract(_date1970);
    ...
    /// 	Contributed by blaumeister, http://www.codeplex.com/site/users/view/blaumeiser
    /// </remarks>
    public static long GetMillisecondsSince1970WithOffset(this DateTime datetime)
    {
        var offset = DateTimeOffset.Now.Offset.TotalHours;
        var ts = datetime.AddHours(-offset).Subtract(_date1970);
        return (long)ts.TotalMilliseconds;
    }
    ...
    /// <summary>
    /// Gets the week number for a provided date time value based on the current culture Settings.Instance.
    ...
    /// <summary>
    /// Returns a value indicating whether the given date is between the two specified dates, ignoring
    /// the time component by default.
    /// </summary>
    public static bool IsBetween(this DateTime date, DateTime start, DateTime end, bool ignoreTime = true)
    {
        if (ignoreTime)
        {
            return date.Date == start.Date || date.Date == end.Date ||
                (date.Date.IsAfter(start.Date) && date.Date.IsBefore(end.Date));
        }
    ...
    public static DateTime GetNextSaturday(this DateTime now)
    {
        if (now.DayOfWeek == DayOfWeek.Saturday)
            return now.Date;
        else return GetNextSaturday(now.AddDays(1));
    }

    public static DateTime GetPreviousFriday(this DateTime now)
    {
        if (now.DayOfWeek == DayOfWeek.Friday)
            return now.Date;
        else return GetPreviousFriday(now.AddDays(-1));
    }
